...
# - saving_camera_v4 - отличается от saving_camera_v2 тем что TAB теперь переключает режим оброза и управления мышеджостиком
# - аналочно тому как в в default_v3
# =====================================================
# --- ВНИМАНИЕ !!! ---
...
# =====================================================
# --- Оси виртуального джойстика (vJoy[0]) (могут быть переназначены по желанию) ---
# - Оси X,Y - оси мышеджоя (крен, тангаж)
# - Ось Z - ось рысканья
# - Ось SLIDER - ось РУД
# =====================================================
# --- Оси виртуального джойстика (vJoy[1]) (могут быть переназначены по желанию) ---
...
if starting:
    ...
    # Клавиши обзора
    VIEW_UP_BUTTON = Key.D5
    VIEW_DOWN_BUTTON = Key.D6
    VIEW_LEFT_BUTTON = Key.D7
    VIEW_RIGHT_BUTTON = Key.D8
    VIEW_BACK_BUTTON = Key.D9

    # Клавиши смещения вверх/вниз
    VIEW_OFFSET_UP_BUTTON = Key.D1
    VIEW_OFFSET_DOWN_BUTTON = Key.D2

    # Клавиши смещения лево/право
    VIEW_OFFSET_LEFT_BUTTON = Key.D3
    VIEW_OFFSET_RIGHT_BUTTON = Key.D4
...
# Кнопка обзора с сохранениям камеры
if (
...
if (
    not is_mouse_vjoy_active
    and keyboard.getKeyUp(MOUSE_VJOY_TOGGLE_BUTTON)
    and keyboard.getKeyUp(MOUSE_VJOY_TOGGLE_SAVE_VIEW_BUTTON)
    and not is_mouse_vjoy_unenable
):
    is_mouse_view_enabled = False
    is_mouse_vjoy_active = True
...
if keyboard.getKeyDown(MOUSE_VJOY_TOGGLE_BUTTON) or keyboard.getKeyDown(MOUSE_VJOY_TOGGLE_SAVE_VIEW_BUTTON) and is_mouse_vjoy_unenable:
    is_mouse_view_enabled = True

# Выключение обзора при выключенном джое
if keyboard.getKeyUp(MOUSE_VJOY_TOGGLE_BUTTON) and keyboard.getKeyUp(MOUSE_VJOY_TOGGLE_SAVE_VIEW_BUTTON) and is_mouse_vjoy_unenable:
    is_mouse_view_enabled = False
