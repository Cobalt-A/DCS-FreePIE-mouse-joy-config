...
# - default_v1 - максимально похожий на мышеджой war thunder'a
# =====================================================
# --- ВНИМАНИЕ !!! ---
...
if starting:
    ...
    # def change_axis(current, target, sensitivity): # значения sensitivity от 0 до 1
	#     new_value = current + (target - current) * sensitivity
	#     flag = abs(current - new_value) < 1
	#     if (flag): return target, flag
	#     return new_value, flag
...
if (
    ...
    vJoy[1].x = new_value
    is_keyboard_offset_x_active = not flag
...
if (
    ...
    vJoy[0].x = new_value
    is_keyboard_joy_x_enabled = not flag
...
if (
    ...
    vJoy[0].y = new_value
    is_keyboard_joy_y_enabled = not flag
...
if (
    ...
    vJoy[0].z = new_value
    is_keyboard_z_enabled = not flag
